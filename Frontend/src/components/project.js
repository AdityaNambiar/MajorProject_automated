...
class Project extends Component {
  ...
  handleProjectCollaborators = async () => {
    try{
      ...
      const {data} = await axios.post(`${url}/getProjectsOfMember`,{},{
        headers:{
          'x-auth-token':localStorage.getItem("x-auth-token")
        }
      });
   ...
   try{
        ...
        var mutationObserver = new MutationObserver(function(mutations) {
          mutations.forEach(function(mutation) {
            let tag = mutation.target.tagName;
            let type = mutation.target.type;
    ...
    // document.getElementsByClassName("action-bar")[0].addEventListener("")
    await this.getFiles()
  ...
  handleBuildDeploy = ()=>{
    ...
    this.props.history.push("/buildanddeploy",{projectid,branchOn})
  }
  ...
  handleDirectDeploy = ()=>{
    ...
    let projectid = this.props.location.state.projectid;
    let branchOn = this.state.branchOn;
    this.props.history.push("/directdeploy",{projectid,branchOn})
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="row ml-2" style={{display:"flex",flexWrap:"nowrap"}}>
          ...
          <button
            ...
            >
              ...
              {access&&(
                ...
                title="Build using jenkins and then deploy"
                onClick={this.handleBuildDeploy}
              >
                Build & Deploy
              ...
              <button
                type="button"
                className="btn btn-outline-info btn-sm ml-2 mt-2 mb-2"
                title="Deploy directly without jenkins build"
                onClick={this.handleDirectDeploy}
              >
                Deploy

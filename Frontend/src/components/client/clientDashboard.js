...
class ClientDashboard extends Component {
  ...
  handleClientDocUpload = async(e) => {
   ...
   if(Object.keys(ipfsFiles).length==0){
        return alert("Select atleast a file")
   }

    if(this.state.clientid==""){
        return alert("You Should have a client to upload to. Please select a client if exists")
    }
    try{

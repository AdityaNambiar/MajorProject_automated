...
class DeployDirect extends Component {
  ...
  handleStartBuild = async ()=>{
   ...
   try{ 
    let {projName,branchName,tagname} = this.state;
    // if(tagname.trim()==""){
    ...
    const {data} =  await axios.post(urls.deployDirectly,{projName,branchName,tagName:tagname},{
        headers:{
    ...
    this.setState({disablebtn:false,buildurlarea:true,urls:data.urls})
     ...
     alert(error);
     this.setState({disablebtn:false,buildurlarea:false})
   } 
  ...
  render() {
    ...
    <div className="container">  
        <div style={{display:"flex",justifyContent:"flex-start" ,paddingLeft:"100px"}}>
            <button className="btn btn-primary btn-sm m-3" disabled={disablebtn} onClick={this.handleStartBuild}>Start Build</button>
            <button className="btn btn-secondary btn-sm m-3" onClick={this.getBuild} disabled={disablebtn}>Existing Build</button>
        </div> 
{/* 
      ...
      id="logarea"
      style={{
        display:`${startbuildarea==false?"none":"block"}`
      }}>
        <h2>{projName}</h2>
          <ProgressBar
          ...
          <hr className="mt-5"/>
          <textarea ref="logsref" value={this.state.postResp} rows="15" className="mt-3 p-5 w-100 bg bg-dark text-light" readOnly>
          </textarea>

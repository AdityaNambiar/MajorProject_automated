...
router.post('/diffFiles', async (req, res) => {
    ...
    try {
        ...
        let response = await main(projName, username, timestamp, branchToUpdate, ref1, ref2, barerepopath, workdirpath, curr_majorHash, url)
        res.status(200).send(response);
    } catch (err) {
        console.log(err);
        res.status(400).send(`(diffFiles) err ${err.name} :- ${err.message}`);
    }
})
